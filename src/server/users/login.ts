...
export async function login(email: string, pass: string) {
  ...
  const res = await client.query<User>(
    `SELECT * FROM users WHERE email = $1 RETURNING email, id`,
    [email]
  );
  ...
  if (await bcrypt.compare(pass, digest)) {
    ...
    async function createSession() {
      // create new token, hash that token.
      const token = uuidv4();
      ...
      const newSession = await client.query<Session>(
        `INSERT INTO sessions (user_id, hashed_access_token, expires_at) VALUES ($1, $2, $3) RETURNING user_id, expires_at`,
